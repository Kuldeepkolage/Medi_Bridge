        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {statCards.map((card) => (
              <div
                key={card.key}
                className={`bg-white border ${card.border} border-gray-200 rounded-2xl p-5 shadow-sm`}
              >
                <div className={`w-10 h-10 ${card.iconBg} ${card.iconColor} rounded-xl flex items-center justify-center mb-3`}>
                  {card.icon}
                </div>
                <p className={`text-3xl font-bold ${card.numColor}`}>{stats[card.key] ?? 0}</p>
